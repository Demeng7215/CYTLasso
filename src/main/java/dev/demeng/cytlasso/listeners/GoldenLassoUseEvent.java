...
import dev.demeng.cytlasso.utils.XMaterial;
...
public class GoldenLassoUseEvent implements Listener {
  ...
  public void onGoldenLassoUse(PlayerInteractEntityEvent e) {
    ...
    if (!e.getPlayer()
        .getInventory()
        .getItemInMainHand()
        .isSimilar(GoldenLassoItem.getGoldenLasso())) return;
    ...
    if (!i.getEconomy().has(e.getPlayer(), i.getSettings().getDouble("cost"))) {
      MessageUtils.tell(e.getPlayer(), i.getSettings().getString("insufficient-balance"));
      return;
    }

    ItemStack spawnEgg =
        XMaterial.valueOf(e.getRightClicked().getType().name() + "_SPAWN_EGG").parseItem();
    ...
    if (owner != null)
      dataLore.add(
          parseData(
              "Owner", Bukkit.getServer().getOfflinePlayer(UUID.fromString(owner)).getName()));
    ...
    i.getEconomy().withdrawPlayer(e.getPlayer(), i.getSettings().getDouble("cost"));
        ...
        .scheduleSyncDelayedTask(
            ...
            () -> {
              if (!e.getPlayer()
                  .getInventory()
                  .getItemInMainHand()
                  .isSimilar(GoldenLassoItem.getGoldenLasso())) {
                e.getPlayer()
                    .sendTitle(
                        MessageUtils.colorize(i.getSettings().getString("exploit-detected-title")),
                        MessageUtils.colorize(
                            i.getSettings().getString("exploit-detected-subtitle")),
                        20,
                        60,
                        20);
                return;
  ...
  private String parseData(String key, String data) {
    return MessageUtils.colorize(
        i.getSettings().getString("key-color")
            + key
            + ": "
            + i.getSettings().getString("value-color")
            + data);
  }
}
