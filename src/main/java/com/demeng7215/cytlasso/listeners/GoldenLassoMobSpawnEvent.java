...
public class GoldenLassoMobSpawnEvent implements Listener {
	...
	public void onGoldenLassoMobSpawnEvent(PlayerInteractEvent e) {
		...
		if (!e.getPlayer().getInventory().getItemInMainHand().getType().name().endsWith("_SPAWN_EGG")) return;
		...
		if (!meta.getDisplayName().contains(MessageUtils.stripColors(i.getSettings().getString("egg-name")
				.replace("%entity%", "")))) return;
